...
{
    $lang = \Illuminate\Support\Facades\Cache::rememberForever('language.' . $abbr, function () use ($abbr) {
...
function getUrlPageByType($type, $locale = null)
{
    ...
    $page = \Illuminate\Support\Facades\Cache::rememberForever($cacheId, function () use ($type, $locale) {
...
function getPhoneIcon($countryCode)
{
    if (file_exists(public_path() . '/images/flags/16/' . strtolower($countryCode) . '.png')) {
        $phoneIcon = '<img src="' . url('images/flags/16/' . strtolower($countryCode) . '.png') . '" style="padding: 2px;">';
    } else {
...
function getMetaTag($tag, $page)
{
    ...
    try {
        ...
        $metaTag = \Illuminate\Support\Facades\Cache::rememberForever($cacheId, function () use ($languageCode, $page) {
